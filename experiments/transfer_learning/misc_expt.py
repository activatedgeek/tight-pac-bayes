...
def makeTrainer(*,dataset='cifar10',num_epochs=2,model=layer13s,data_dir=None,
                bs=50,lr=.1,optim=SGD,device='cuda',trainer=Classifier,aug=True,projector=RoundedDoubleKron,
...
if __name__=='__main__':
        ...
        # cfg_spec.update({
        #     'd':4000,'study_name':'network_comparison','num_epochs':2,'lr':.1,
        #     'dataset':'cifar10', 'model':layer13s,
        #     'net_config':{'base_width':16},'aug':True,
        # })
        ...
        cfg_spec = copy.deepcopy(cfg_spec)
        cfg_spec.update({
            'd':10000,'study_name':'misc_comparison2','num_epochs':40,'lr':.1,
            'dataset':'cifar10', 'model':layer13s,'projector':RoundedDoubleKron,
            'net_config':{'base_width':8},'aug':False,
        })
        name = cfg_spec.pop('study_name')
        thestudy = Study(Trial,cfg_spec,study_name=name,base_log_dir=cfg_spec['trainer_config'].get('log_dir',None))
        # thestudy.run(new_config_spec=cfg_spec)
        # cfg = copy.deepcopy(cfg_spec)
        # cfg['aug'] = True
        # thestudy.run(new_config_spec=cfg)
        cfg = copy.deepcopy(cfg_spec)
        cfg['projector'] = CombinedRDKronFiLM
        thestudy.run(new_config_spec=cfg)
        cfg = copy.deepcopy(cfg_spec)
        cfg['net_config']['base_width'] = 16
        thestudy.run(new_config_spec=cfg)
